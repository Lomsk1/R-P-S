...
rounds.forEach((el, i) => el.addEventListener('click', function(){
    const math = Math.trunc(Math.random() * 5) +1;
    console.log(math);
    document.querySelector('.game').classList.add('hidden');
    document.querySelector('.step__two').classList.remove('hidden');
    function tabloo(){
    ...
    function first(){
        setTimeout(() =>document.querySelector('.circle').classList.add('hidden'),1000);
        setTimeout(() =>scissors(),1000);
        setTimeout(() => tabloo(), 1500)
    }
    function second(){
        setTimeout(() =>document.querySelector('.circle').classList.add('hidden'),1000);
    ...
    function third(){
        setTimeout(() =>document.querySelector('.circle').classList.add('hidden'),1000);
        setTimeout(() =>spock(),1000);
        setTimeout(() => tabloo(), 1500)
    }
    function forth(){
        setTimeout(() =>document.querySelector('.circle').classList.add('hidden'),1000);
        setTimeout(() =>lizard(),1000);
        setTimeout(() => tabloo(), 1500)
    }
    function fifth(){
        setTimeout(() =>document.querySelector('.circle').classList.add('hidden'),1000);
    ...
    // document.querySelector(`.image`).insertAdjacentElement('afterbegin', el);
  
    if( i+1 === 1){
        one.insertAdjacentHTML('afterbegin',scissors1)
        if(math === 1){
            first()
            tabloP.textContent = 'TRY AGAIN';
        }
        if(math === 2){
            second()
            tabloP.textContent = 'YOU WIN';
            currentNUmber++
            setTimeout(() => scores(), 1500)
        }
        if(math === 3){
            third()
            tabloP.textContent = 'YOU LOSE';
            currentNUmber--
            setTimeout(() => scores(), 1500)
        }
        if(math === 4){
    ...
    if(i+1 === 2){
        one.insertAdjacentHTML('afterbegin',paper1)
        if(math === 1){
            first();
            tabloP.textContent = 'YOU LOSE';
            currentNUmber--
    ...
    if( i+1=== 3){
        one.insertAdjacentHTML('afterbegin',spock1)
        if(math === 1){
            first();
            tabloP.textContent = 'YOU WIN';
            currentNUmber++
    ...
    if( i+1=== 4){
        one.insertAdjacentHTML('afterbegin',lizard1)
        if(math === 1){
            first();
            tabloP.textContent = 'YOU LOSE';
            currentNUmber--
    ...
    if( i+1=== 5){
        one.insertAdjacentHTML('afterbegin',rock1)
        if(math === 1){
            first();
            tabloP.textContent = 'YOU WIN';
            currentNUmber++
...
again.addEventListener('click', function(){
  document.querySelector('.step__two').classList.add('hidden');
  document.querySelector('.game').classList.remove('hidden');
  document.querySelector('.circle').classList.remove('hidden');
  tablo.classList.add('hidden')

  one.removeChild(one.firstElementChild)
  two.removeChild(two.firstElementChild)
})